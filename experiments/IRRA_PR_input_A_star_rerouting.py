...
from src.algorithms.IRRA import IRRA_PR
import time
import copy
import statistics
import json
...
short_circuit_count = []
results = []
boolian_variation = [[False, True]]
technique_names = ["IRRA_A*_A*"]
best_output = []
best_chip = None
lowest_cost = inf
...
for i, reroute_type in enumerate(boolian_variation):
    while n_runs < 1000:
        ...
        chip0 = chip_og
        irra_irra = IRRA_PR(chip= chip0, iterations=1, intersection_limit= 0, acceptable_intersection=100, simulated_annealing = reroute_type[0], A_star_rerouting= reroute_type[1], start_temperature = 750, temperature_alpha = 0.99)
...
output_file = 'output/PR_input/chip2w7_astar_rerouting_1000_final.json' 
with open(output_file, 'w') as file:
    json.dump(results, file, indent=4)

best_chip.save_output('chip2w7_PR_1000_final.csv')
best_chip.show_grid('chip2w7_PR_1000_final.html', best_algorithm)